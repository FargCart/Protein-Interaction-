...
# def findChain(chain):
#     for resid in chain:
#         atomChain = str(chain).split("=")
#         atomChain = str(atomChain[1]).split('>')
#     return(atomChain)



def calc_inter(chain,
        hydrophobic_dict,
        amino_type,
        filename,
        distmin=0,
    ...
    atomChain = str(atomChain[1]).split('>')
    if csv:
        to_csv = ["Chain , RES1 , idRES1 , Chain , RES2 , idRES2 , dist(Angstrom)\n"]
                ...
                if res:
                    if (resid1, resid2) not in found and (
                        ...
                        if csv:
                            ...
                            to_print1 = '{:<6}'.format(str(atomChain[0]))
                            # print(to_print)
                            # time.sleep(.5)
                             # Putting in Residue 1 and ID
                            to_print = to_print1 + '{:<5}'.format(str(resid1).split()[1]) + ", " + '{:<7}'.format(
                                str(resid1).split()[3].split("=")[1])
                            #
                            # time.sleep(.5)


                            # Putting in Residue 2 and ID
                        ...
                        to_print = "| " + to_print1+ '|' + '{:<6}'.format(str(resid1).split()[1]) + "| " + '{:<7}'.format(
                            str(resid1).split()[3].split("=")[1])



                        # Formatting List
                        to_print = to_print + "| " + '{:<6}'.format(str(resid2).split(
                        )[1]) + "| " + '{:<5}'.format(str(resid2).split()[3].split("=")[1])


                        # Formatting Distance Printing
                        to_print = to_print + "| " + \
                            str('{:06.2f}'.format(res)) + "        |"
                        print(to_print)
